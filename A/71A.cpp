...
and between them we write the number of letters between the first and the last letters. That 
...
At that all too long words should be replaced by the abbreviation and the words that are 
not too long should not undergo any changes.
...
Output
Print n lines. The i-th line should contain the result of replacing of the i-th word from the 
input data. */
...
int main() {
	...
	while (t--) {
		...
		if (word.length() > 10) {
			...
			new_word = first + to_string(word.length() - 2) + last;
			cout << new_word << endl;
		}
		else
			cout << word << endl;
